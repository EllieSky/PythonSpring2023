...
from pages.view_emp_list import ViewEmpListPage


def get_greeting(browser):
    return browser.find_element(By.ID, 'welcome').text

#then copied and pasted to view_emp_list.py
...
def get_table_data(browser, current_url):
    return browser.find_elements(By.CSS_SELECTOR, f'#resultTable tr>td:nth-child({current_url})')


def logout(browser, current_url):
...
class MyTestCase(unittest.TestCase):
    def test_search_by_job_title(self):
        ...
        # login
        self.sign_in.login() # added self.login in SetUp
        # login(browser)
        ...
        # emp_list = ViewEmpListPage(browser) moved it to SetUp
    ...
    def test_search_by_name(self):
        ...
        all_l_name_values = get_table_data(self.browser, 4)

        num_rows = len(all_fname_values)
        for i in range(num_rows):
            # self.assertIn('Jo', (all_fname_values[i].text + all_fname_values[i].text))
            # self.assertTrue('Jo' in all_fname_values[i].text or 'Jo' in all_l_name_values[i].text)
            self.assertIn('Jo', f'{all_fname_values[i].text} {all_l_name_values[i].text}')
    ...
    def setUp(self) -> None:
        ...
        self.emp_list = ViewEmpListPage(browser)
